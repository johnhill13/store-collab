...
const Categories = ({ categories, onClickCategory }) => {
    const listCategory = categories.map(category => (
        < div className="col-lg-3 col-md-4 col-sm-6 col-10 my-2 text-center" key={category.name} onClick={onClickCategory} >
        ...
        <>
            ...
            <div className="container">
                ...
                <Link className="nav-link" to="/categories/productlist">
                    <div className="row align-items-center justify-content-center">{listCategory}</div>
                </Link>
            </div>
        </>
    );
